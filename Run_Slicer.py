...
class Slicer():
	...
	VALID_TAGS = ["svg", "circle", "ellipse", "line", "path", "polygon", "polyline", "rect"]
	...
	def init_analyzer (self, res = False, err = False):
		self.analyzer_root = Slicer_Analyzer.analysis_methods(self, res, err, self.FLOAT_DECIMAL_PNTS)
		self.analyzer = {
		...
		if self._paper_coordinates is False:
			...
			for g in range(0, len(self._paper_coordinates)):
				...
				for c in range(0, len(group)):
					coordinate = group[c]
					coordinate[1] += min_y
					dist = self.dist(coordinate[0], coordinate[1], current_position[0], current_position[1])
					deg = False
					if dist != 0:
						if c == 0:
							fl.write("P 0\n")
						...
						if float(coordinate[0] - current_position[0]) == 0:
							...
							slope = float(coordinate[1] - current_position[1]) / float(coordinate[0] - current_position[0])
							deg = math.atan(slope)
					if deg is not False:
						temp = direction
						direction = deg
						deg -= temp
						fl.write("M " + (str("%." + str(self.FLOAT_DECIMAL_PNTS) + "f") % deg) + " " + (str("%." + str(self.FLOAT_DECIMAL_PNTS) + "f") % dist) + "\n")
...
# 			fl.write("P 0")
	...
	def clean_raw (self):
		...
		self._vectors_arr = re.findall(r'(<[^a-zA-Z/]?(' + "|".join(self.VALID_TAGS) + ')[^>]*>)', self._raw_file)
	...
	def init_svg (self, obj):
		arr_args = re.findall(r'([a-zA-Z]+)[ ]*=[ ]*[\'\"]([^(\"|\')]*)[\'\"]', obj[0])
		args = {}
		for x in arr_args:
			args[x[0].lower()] = x[1]
		self._svt_tag = obj[0]
		w = 0
		h = 0
		if "width" not in args or "height" not in args: # Dimensions not specified | Response: use default dimensions _WIDTH and _HEIGHT
		...
		f = 0
		if not re.match("^([0-9]+[\.]?[0-9]*)(" + self.preg_units + ")$", w): # if width attribute is invalid
		...
		if f != 2:
			if not re.match("^([0-9]+[\.]?[0-9]*)(" + self.preg_units + ")$", h): # if height attribute is invalid
		...
		if f == 1:
			...
			if h[0] / w[0] > (self.HEIGHT - 2 * self.DEFAULT_MARGIN) / (self.WIDTH - 2 * self.DEFAULT_MARGIN): # Scale based on height
				temp = h[0]
		...
		while index + 1 < _len:
			...
			if len(x) > 1:
				...
				while y < cond or cond is False:
					b = [self.coordinate_check_out(x[y]), self.coordinate_check_out(x[y+1])]
					intersections = self.locate_intersection_bound(x[y], x[y+1])
					intersections_disp = []
					if intersections != False:
						for w in intersections:
							intersections_disp.append(self.dist(x[y][0], x[y][1], w[0], w[1]))
					if b[0] and b[1]:
						if intersections == False:
							if y+2 < len(x):
							...
							min_index = intersections_disp.index(min(intersections_disp))
							x[y] = intersections[min_index]
							x.insert(y+1, intersections[(0 if min_index == 1 else 1)])
						...
						if intersections == False or len(intersections) != 1:
							...
							if y + 1 >= len(x) - 1:
								...
								self._coordinate_grid.insert(index+1,x[y+2:])
								x = x[0:y+2]
	...
	def shorten_toolpaths (self):
		...
		for x in range(0, len(self._coordinate_grid) - 1):
			...
			for y in range(x+1, len(self._coordinate_grid)):
				temp = min([
					self.dist(self._coordinate_grid[x][0][0],self._coordinate_grid[x][0][1],self._coordinate_grid[y][0][0],self._coordinate_grid[y][0][1]), 
					self.dist(self._coordinate_grid[x][0][0],self._coordinate_grid[x][0][1],self._coordinate_grid[y][-1][0],self._coordinate_grid[y][-1][1]), 
					self.dist(self._coordinate_grid[x][-1][0],self._coordinate_grid[x][-1][1],self._coordinate_grid[y][0][0],self._coordinate_grid[y][0][1]), 
					self.dist(self._coordinate_grid[x][-1][0],self._coordinate_grid[x][-1][1],self._coordinate_grid[y][-1][0],self._coordinate_grid[y][-1][1]), 
				])
				if min_dist == False or temp < min_dist:
					index = x
					min_dist = temp
	...
	def set_paper (self, width=False, height=False, margins=False, lock_ratio=False):
		if width is False:
			width = self.WIDTH
		if height is False:
			height = self.HEIGHT
		...
		for x in range(0, len(self._coordinate_grid)):
			...
			for c in range(0, len(self._coordinate_grid[x])):
				...
				self._paper_coordinates[x][c] = [(self._coordinate_grid[x][c][0] - self._viewbox[0]) * self._scale_x * scale_x + margins + x_disp, (self._coordinate_grid[x][c][1] - self._viewbox[1]) * self._scale_y * scale_y + margins + y_disp]
