import Slicer as Run_Slicer
import os
import Unit_Converter
import re

# Interface for user to convert vector image file to several sets of coordinates

# Originally programmed on mac so system clear command may be different if
# run on Windows
...
while dimensions_okay is False:
    ...
    for x in range(0, 3):
        ...
        while flag >= 0:
            ...
            input = raw_input(
                dimension_labels[x] +
                " of paper (press enter to use default): ")
            if input == "":
                ...
                try:
                    ...
                    dimensions[x] = Unit_Converter.convert(
                        float(temp[0]), temp[1])
                ...
                if f is True:
                    flag = -1
    t = "Are the dimensions of " + str(dimensions[0]) + "mm x " + str(dimensions[1]) + "mm with " + (
        "default" if dimensions[2] is False else str(dimensions[2]) + "mm") + " margins right? [y/n]:"
...
print "\n"
input = raw_input(
    "Filename, without .acode extension, to export to (default is \"autopen\"): ")
if input == "":
    slicer_obj.export()
else:
