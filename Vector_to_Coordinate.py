...
# Class that contains methods for converting vector shape into set of
# coordinates
...
class analysis_methods():
    ...
    def circle(self, args):
        ...
        point_array = [[]]
        while deg < 2 * math.pi:
            point_array[0].append(
                [math.cos(deg) * args["r"] + args["cx"], math.sin(deg) * args["r"] + args["cy"]])
            deg += iv
        return point_array
    ...
    def approx_ellipse(self, n1, n2, a, b, xi, yi):
        ...
        if math.fabs(dist - self.RESOLUTION) / \
                self.RESOLUTION <= self.RESOLUTION_ERR:  # condition
            return [xx, yx]
        else:
    ...
    def ellipse(self, args):
        ...
        while i >= 0:  # Flip quadrant 1 over x-axis
            if i != l and i != 0:
                point_arr.append([point_arr[i][0] + args["cx"],
                                  point_arr[i][1] * -1 + args["cy"]])
    ...
    def calc_quadratic_bezier(t, arg, w):
        x = (w[0] - 2 * arg[0] + arg[2]) * math.pow(t, 2) + \
            (2 * arg[0] - 2 * w[0]) * t + w[0]
        y = (w[1] - 2 * arg[1] + arg[3]) * math.pow(t, 2) + \
            (2 * arg[1] - 2 * w[1]) * t + w[1]
        return [x, y]
    ...
    def generate_bezier(self, w, arg, first_coordinate):
        ...
        while t == 0 or dist > self.RESOLUTION:
            approximation = self.approx_bezier(
                "cubic",
                t,
                1,
                arg,
...
# 		w.append([arg[4], arg[5]])
    ...
    def arc_to_bezier(x1, y1, x2, y2, fa, fs, rx, ry, phi):
        ...
        cc = analysis_methods.get_arc_center(
            x1, y1, x2, y2, fa, fs, rx, ry, sin_phi, cos_phi)
        ...
        for i in range(0, segments):
            result.append(
                analysis_methods.approximate_unit_arc(
                    theta1, delta_theta))
            theta1 += delta_theta
        ...
        for x in _args:
            ...
            if x[1] == "M":
                        ...
                        _args.insert(index +
                                     1, [cmd +
                                         ...
                                         " ".join([str('{:.' +
                                                       ...
                                                       'f}').format(n) for n in arg[0:2]]), cmd])
                        cmd = "l" if rel else "L"
                ...
                if x[1] == "H":
                    if len(arg) != 1:
                        raise ValueError(
                            "Invalid 'H' value in path SVG element")
                    else:
                        if not rel:
                    ...
                    if len(arg) != 1:
                        raise ValueError(
                            "Invalid 'V' value in path SVG element")
                    else:
                        if not rel:
                    ...
                    if len(arg) != 7:
                        raise ValueError(
                            "Invalid 'A' value in path SVG element")
                    else:
                        if rel:
                            arg[5] += w[-1][0]
                            arg[6] += w[-1][1]
                        ...
                        bezier_approximations = self.arc_to_bezier(
                            w[-1][0], w[-1][1], arg[5], arg[6], arg[3], arg[4], arg[0], arg[1], arg[2])
                        cmd = "M " + (str("%." + str(self.FLOAT_DECIMAL_PNTS) + "f") % w[-1][0]) + " " + (
                            str("%." + str(self.FLOAT_DECIMAL_PNTS) + "f") % w[-1][1]) + " "

                        for b in bezier_approximations:
                            b = [str("%." +
                                     str(self.FLOAT_DECIMAL_PNTS) +
                                     "f") %
                                 number for number in b]
                            cmd += "C " + " ".join(b[2:]) + " "
                        w.extend(self.path({"d": cmd})[0][1:])
                else:
                    if x[1] == "C" or x[1] == "S":
                        if ((len(arg) < 6 or len(arg) % 6 != 0) and x[1] == "C") or (
                                (len(arg) < 4 or len(arg) % 4 != 0) and x[1] == "S"):
                            raise ValueError(
                                "Invalid 'C/S' value in path SVG element")
                        else:
                            if x[1] == "S":
                                if len(arg) > 4:
                                    skip_flag = True
                            ...
                            if x[1] == "C":
                                if len(arg) > 6:
                                    ...
                                    self.generate_bezier(
                                        w, arg, first_coordinate)

                    if x[1] == "Q" or x[1] == "T":
                        if ((len(arg) < 4 or len(arg) % 4 != 0) and x[1] == "Q") or (
                                (len(arg) < 2 or len(arg) % 2 != 0) and x[1] == "T"):
                            raise ValueError(
                                "Invalid 'Q' value in path SVG element")
                        else:
                            ...
                            if x[1] == "Q":
                                if len(arg) > 4:
                                    ...
                                    while t == 0 or dist > self.RESOLUTION:
                                        ...
                                        w.append(approximation[1])
                                        dist = self.dist(
                                            arg[2], arg[3], approximation[1][0], approximation[1][1])
    ...
    # 								w.append([arg[2], arg[3]])
                    ...
                    if x[1] == "L":
                        if len(arg) > 2 and len(arg) % 2 == 0:
                            ...
                            raise ValueError(
                                "Invalid 'L' value in path SVG element")
                        else:
                            self.convert_rel(rel, arg, w)
                            w.append([arg[0], arg[1]])
            if not skip_flag:
